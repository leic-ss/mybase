...
cs_ip_array=(192.168.56.101 192.168.56.103)
cs_port_array=(5198)

ds_ip_array=(192.168.56.101 192.168.56.103)
...
if [ x$1 == x"deploy" ];then
	...
	for ds_port in "${ds_port_array[@]}"; do
		...
		cp -rf ${bin_dir}/scripts $work_dir/dataserver_${ds_port}
		cp -f $bin_dir/scripts/daemo_dataserver.sh $work_dir/dataserver_${ds_port}/scripts
		# cp -f $bin_dir/scripts/set_shm.sh $work_dir/dataserver_${ds_port}/scripts
		cp -rf ${bin_dir}/scripts/monitor $work_dir/dataserver_${ds_port}/scripts
		cp -rf ${bin_dir}/scripts/common $work_dir/dataserver_${ds_port}/scripts
		# chmod +x $work_dir/dataserver_${ds_port}/scripts/common/srm
	...
	for cs_port in "${cs_port_array[@]}"; do
		...
		# cp -f $tair_bin_dir/bin/tair_server $work_dir/dataserver_${cs_port}/bin
		cp -f $bin_dir/bin/configserver $work_dir/configserver_${cs_port}/bin
		# cp -f $bin_dir/bin/client $work_dir/configserver_${cs_port}/bin
		# cp -f $bin_dir/bin/cst_monitor $work_dir/configserver_${cs_port}/bin

		# cp -rf ${bin_dir}/scripts $work_dir/dataserver_${cs_port}
		cp -f $bin_dir/scripts/daemo_configserver.sh $work_dir/configserver_${cs_port}/scripts
		# cp -f $bin_dir/scripts/set_shm.sh $work_dir/configserver_${cs_port}/scripts
		# cp -rf ${bin_dir}/scripts/monitor $work_dir/configserver_${cs_port}/scripts
		...
		write_configserver_conf $cs_port
