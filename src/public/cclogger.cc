...
const char * const CCLogger::_errstr[] = {"SYSM", "FATL", "ERRO","WARN","INFO","DEBG", "TRAC"};
...
{
    if ( _level < 0 || _level > (int32_t)(sizeof(_errstr)/sizeof(char*)) ) {
        return "unknow";
    }
...
void CCLogger::rotateLog(const char *filename, const char *fmt) 
{
    ...
    if (access(filename, R_OK) == 0) {
        ...
        if (fmt != NULL) {
            ...
            rc = snprintf(oldLogFile, sizeof(oldLogFile), "%s.%s", filename, tmptime);
            rc = snprintf(old_wf_log_file, sizeof(old_wf_log_file), "%s.%s", wf_filename, tmptime);
        } else {
            rc = sprintf(oldLogFile, "%s.%04d%02d%02d%02d%02d%02d",
                filename, tm.tm_year+1900, tm.tm_mon+1, tm.tm_mday,
                tm.tm_hour, tm.tm_min, tm.tm_sec);
            rc = snprintf(old_wf_log_file, sizeof(old_wf_log_file), "%s.%04d%02d%02d%02d%02d%02d",
