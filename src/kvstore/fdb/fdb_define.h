...
static const int32_t sFdbKeyBucketSize = 3;
static const int32_t sFdbKeyAreaSize = 2;
static const int32_t sMaxBucketNumber = (1 << 24) - 2;

class FdbKey
{
public:
    FdbKey() : data_(nullptr), data_size_(0), alloc_(false) {}
    FdbKey(const char* key_data, int32_t key_size, int32_t bucket_number, int32_t area)
...
{
public:
    FdbItem() : meta_(), data_(nullptr), data_size_(0), alloc_(false) {}
    ~FdbItem() { destroy(); }
